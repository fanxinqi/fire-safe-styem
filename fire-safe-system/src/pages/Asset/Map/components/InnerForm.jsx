...
import { queryProductType, add } from '../service';
...
const InnerForm = (props) => {
  ...
  const handleOk = () => {
    ...
    console.log(curreTag.getData());
    setVisible(false);
  };
  const changeHandle = (e) => {
    const productTypeItem = productType.find((item) => item.productTypeId == e.target.value);
    setProductTypeName(productTypeItem.productType);
    setProductTypeId(productTypeItem.productTypeId);
    window.product = {
      productTypeName: productTypeItem.productType,
  ...
  // const newTag = (position, text) => {
  //   const tag = new Taggd.Tag(position, text, {
  //     src: markIcon,
  ...
  //     var tag = newTag(position, window.product.productTypeName);
  //     taggd.addTag(tag);
  //     setCurreTag(tag);
  //     // setVisible(true);
  //   });
  ...
  return (
    ...
    >
      ...
      <Form.Item label="存放点" name="locationId">
        <LocationSelect mode={'tag'} />
      </Form.Item>

      <Form.Item label="地图名字" name="mapName">
        <Input />
      </Form.Item>
      <Form.Item label="上传图片" name="imgUrl">
